...
pair<unsigned short, unsigned short> Sudoku_Solver::solve_helper(size_t depth) {
	...
	pair<unsigned short, unsigned short> carry;
	for (auto it = domain.begin(); it != domain.end(); ++it) {
		...
		if (sudoku.get_num_blank() == 0) {
			throw true; //catch outside recursion
		}

		carry = solve_helper(depth+1);
		auto carry_row = carry.first;
		auto carry_col = carry.second;
		//if current block is part of the cumulative actions that caused the conflict
		if (sudoku.cumulative_conflict_set_find(carry_row, carry_col, row, col)) {
			//we have moved up the depth to the latest conflict-related block

			//absord conflict set and cumulative conflict set
			sudoku.cumulative_conflict_set_propagate(row, col, carry_row, carry_col);
			sudoku.conflict_set_propagate(row, col, carry_row, carry_col);
			sudoku.cumulative_conflict_set_erase_self(row, col);

			unset_val_and_update(row, col); //undo
			continue; //continue search with next value for this block
		} else {
			unset_val_and_update(row, col); //undo
			return carry; //move back up a depth until we find a conflict-related block
		}
	}
	
	//if reached here, then it indicates that all values for this block
	//at this current depth eventually led to a conflict eventhough it was conflict-related
	return make_pair(row,col); //move back up a depth until we find the next latest conflict-related block
}
...
bool Sudoku_Solver::solve() {
	...
	try {
		auto result = solve_helper(); //DEBUG
